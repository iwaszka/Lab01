...
author: "Aleksandra Iwaszkiewicz"
date: "`r Sys.Date()`"
...
```
```{r Q3}
 visitors %>% arrange(n_2022)
```
Ans.: Museum of the Jewellery Quarter
```{r Q4}
...
```{r Q5}
 visitors %>% filter(setting == "O", region == "Yorkshire and the Humber", admission == "Members", n_2022 > 100000)
```
...
NA stands for not available, appears in the answer to the previous question because certain values in the n_2021 column are not available and appear as NA in the table column, so a mean or median cannot be calculated and the only response is NA
...
na.rm according to the mean help page: a logical evaluating to TRUE or FALSE indicating whether NA values should be stripped before the computation proceeds.
na.rm is a way of removing NA values from the calculation of the mean and median responding to TRUE or FAlSE, true => remove and false => not remove
...
The difference stems from the mean being calculated using all of the available values, so any extremely high values skew the calculation of the mean causing it to be extremely high. This means that there are a number of attractions that experience numbers of visitors that are significantly larger than the rest of the attractions 
